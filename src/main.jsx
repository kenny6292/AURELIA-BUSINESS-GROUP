...
function AdminPage({onSignOut,go}) {
  const [data,setData]=React.useState(null),[error,setError]=React.useState('');
  React.useEffect(()=>{request('/admin/overview').then(setData).catch(e=>setError(e.message))},[]);
  if(error)return <div className="app-page auth-page"><div className="auth-panel"><p className="form-error">{error}</p><button className="button dark-button" onClick={onSignOut}>Sign out</button></div></div>;
  ...
  return <div className="app-page dashboard-page"><DashboardNav title="Administration" onSignOut={onSignOut} go={go} admin/><main className="dashboard-content"><div className="dashboard-intro"><div><p className="section-label">ADMINISTRATION</p><h1>Platform <em>overview.</em></h1><p className="muted">Live data from the Aurelia PostgreSQL database.</p></div></div><div className="metric-grid">{items.map(([label,value,Icon])=><article className="metric-card" key={label}><Icon/><span>{label}</span><strong>{value}</strong></article>)}</div><section className="dashboard-card admin-table"><p className="section-label">LEAD PIPELINE</p><h2>Recent enquiries</h2><div className="table-row table-head"><span>Name</span><span>Email</span><span>Status</span><span>Date</span></div>{data.enquiries.map(item=><div className="table-row" key={item.id}><span>{item.name}</span><span>{item.email}</span><span>{item.status}</span><span>{new Date(item.created_at).toLocaleDateString()}</span></div>)}</section></main></div>;
}

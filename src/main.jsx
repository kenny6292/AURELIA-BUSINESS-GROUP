...
function AdminPage({onSignOut,go}) {
  const [data,setData]=React.useState(null),[properties,setProperties]=React.useState([]),[users,setUsers]=React.useState([]),[error,setError]=React.useState(''),[busy,setBusy]=React.useState(false);
  const [newProperty,setNewProperty]=React.useState({title:'',location:'',type:'',price:'',currency:'USD',description:''});
  const load=async()=>{try{const [overview,props,usr]=await Promise.all([request('/admin/overview'),request('/admin/properties'),request('/admin/users')]);setData(overview);setProperties(props.properties);setUsers(usr.users);setError('')}catch(e){setError(e.message)}};
  ...
  const removeProperty=async id=>{if(!window.confirm('Delete this property?'))return;setBusy(true);try{await request('/admin/properties/'+id,{method:'DELETE'});await load()}catch(e){setError(e.message)}finally{setBusy(false)}};
  ...
  return <div className="app-page dashboard-page"><DashboardNav title="Administration" onSignOut={onSignOut} go={go} admin/><main className="dashboard-content">
    ...
    <div className="dashboard-grid">
      <section className="dashboard-card"><div className="card-head"><div><p className="section-label">PROPERTY INVENTORY</p><h2>Add property</h2></div></div><form className="admin-form" onSubmit={createProperty}><input required value={newProperty.title} onChange={e=>setNewProperty({...newProperty,title:e.target.value})} placeholder="Property title"/><input required value={newProperty.location} onChange={e=>setNewProperty({...newProperty,location:e.target.value})} placeholder="Location"/><input required value={newProperty.type} onChange={e=>setNewProperty({...newProperty,type:e.target.value})} placeholder="Type"/><input required type="number" min="0" value={newProperty.price} onChange={e=>setNewProperty({...newProperty,price:e.target.value})} placeholder="Price"/><textarea value={newProperty.description} onChange={e=>setNewProperty({...newProperty,description:e.target.value})} placeholder="Description"/><button disabled={busy} className="button dark-button">{busy?'Saving…':'Add property'} <ArrowUpRight size={15}/></button></form></section>
